  ...
  <div class="patent-management">
        ...
        <el-card shadow="hover" class="audit-card">
          <div class="audit-entrance">
            <div class="audit-info">
              ...
              <div>
                <div class="audit-title">待审核专利</div>
    ...
    <el-card shadow="never" class="main-card">
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item label="专利名称">
          <el-input v-model="searchForm.patentTitle" placeholder="请输入" clearable style="width: 200px" />
        </el-form-item>
        
        <el-form-item label="专利类型">
          <el-select v-model="searchForm.patentType" placeholder="全部" clearable style="width: 130px">
            <el-option label="全部" value="" />
      ...
      >
          ...
          <template #default="{ row }">
            <div class="expand-content">
              <el-descriptions :column="2" border size="small">
                ...
                <el-descriptions-item label="专利名称" :span="2">{{ row.patentTitle }}</el-descriptions-item>
                <el-descriptions-item label="专利类型">{{ row.patentType }}</el-descriptions-item>
                <el-descriptions-item label="专利号">{{ row.patentNumber || '-' }}</el-descriptions-item>
                <el-descriptions-item label="发明人排名">{{ row.inventorRank }}</el-descriptions-item>
                <el-descriptions-item label="专利状态">{{ row.patentStatus }}</el-descriptions-item>
                <el-descriptions-item label="申请时间">{{ row.applicationDate }}</el-descriptions-item>
                <el-descriptions-item label="授权时间">{{ row.authorizationDate || '-' }}</el-descriptions-item>
                <el-descriptions-item label="合作发明人" :span="2">{{ row.coInventors || '-' }}</el-descriptions-item>
                <el-descriptions-item label="专利摘要" :span="2">{{ row.abstract || '-' }}</el-descriptions-item>
        ...
        <el-table-column prop="patentTitle" label="专利名称" min-width="200" show-overflow-tooltip />
        <el-table-column prop="patentType" label="类型" width="100" align="center" />
        <el-table-column prop="patentNumber" label="专利号" width="150" align="center" />
        <el-table-column prop="patentStatus" label="状态" width="100" align="center" />
        <el-table-column prop="inventorRank" label="排名" width="70" align="center" />
    ...
    >
      <el-form :model="auditForm" label-width="100px">
        ...
        <el-form-item label="专利名称">
          <el-input v-model="currentRow.patentTitle" disabled />
        </el-form-item>
        <el-form-item label="专利类型">
          <el-input v-model="currentRow.patentType" disabled />
        </el-form-item>
    ...
    >
      <el-descriptions :column="2" border v-if="currentRow">
        ...
        <el-descriptions-item label="专利名称" :span="2">{{ currentRow.patentTitle }}</el-descriptions-item>
        <el-descriptions-item label="专利类型">{{ currentRow.patentType }}</el-descriptions-item>
        <el-descriptions-item label="专利号">{{ currentRow.patentNumber || '-' }}</el-descriptions-item>
        <el-descriptions-item label="发明人排名">{{ currentRow.inventorRank }}</el-descriptions-item>
        <el-descriptions-item label="专利状态">{{ currentRow.patentStatus }}</el-descriptions-item>
        <el-descriptions-item label="申请时间">{{ currentRow.applicationDate }}</el-descriptions-item>
        <el-descriptions-item label="授权时间">{{ currentRow.authorizationDate || '-' }}</el-descriptions-item>
        <el-descriptions-item label="合作发明人" :span="2">{{ currentRow.coInventors || '-' }}</el-descriptions-item>
        <el-descriptions-item label="专利摘要" :span="2">{{ currentRow.abstract || '-' }}</el-descriptions-item>
...
import { getAllPatents, auditPatent, deletePatent } from '@/api/competition'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getAllPatents()
    console.log('专利数据返回:', res)
...
const filterAndPaginate = () => {
  ...
  if (searchForm.patentTitle) {
    filtered = filtered.filter(item => item.patentTitle?.includes(searchForm.patentTitle))
  }
  if (searchForm.patentType) {
    filtered = filtered.filter(item => item.patentType === searchForm.patentType)
  }
